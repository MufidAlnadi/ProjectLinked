...
const CheckoutForm: React.FC<CheckoutProps> = ({ application }) => {
  ...
  const handleSecurityCodeChange = (
    ...
    const inputSecurityCode = event.target.value;
    const sanitizedSecurityCode = inputSecurityCode.slice(0, 3);
    event.target.value = sanitizedSecurityCode;
  };
  const handleCardNumber = (event: React.ChangeEvent<HTMLInputElement>) => {
    const inputSecurityCode = event.target.value;
    const sanitizedSecurityCode = inputSecurityCode.slice(0, 14);
    event.target.value = sanitizedSecurityCode;
  };
  const handleData = (event: React.ChangeEvent<HTMLInputElement>) => {
    const inputSecurityCode = event.target.value;
    const sanitizedSecurityCode = inputSecurityCode.slice(0, 2);
    event.target.value = sanitizedSecurityCode;
  };
